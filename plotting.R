...
age_dist = function(first, method = 'casecontrol'){
  ...
  if(method == 'ageonset'){
    ...
    return(ggplot(first[which(first$Status=="Case"),]) + geom_histogram(aes(x=Age, y = ..density..), position='identity', binwidth = 2, fill = "#00BFC4", alpha = 0.7) +
             geom_density(aes(x=Age)) + geom_vline(xintercept = controls, color = "#F8766D", size=3) + ylab("Count") + 
             ggtitle("Age Distribution") + all_theme + theme(legend.position = "none"))
  }
  
  return(ggplot(first) + geom_histogram(aes(x=Age, y = ..density.., fill=Status), position='identity', binwidth = 2, alpha = 0.7) + 
    geom_density(aes(x=Age)) + ylab("Count") + ggtitle("Age Distribution") + all_theme)
